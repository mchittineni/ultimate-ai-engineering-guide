"""Shared helpers for reading the AI Engineering question vault.
...
def load_topics(root: Path = REPO_ROOT) -> list[Topic]:
    topics: list[Topic] = []
    meta_dict = topic_meta()
    for directory, meta_entry in meta_dict.items():
        ...
        topic = Topic(
            ...
            group=str(meta_entry.get("group", "")),
        ...
        for md in md_files:
            ...
            if not body and meta.get("title"):
                body = f"# {meta['title']}\n"
